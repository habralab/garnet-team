...
public class ProjectTaskOpenCommand
{
    ...
    public async Task<Result<ProjectTaskEntity>> Execute(CancellationToken ct, string taskId)
    {
        ...
        {
            return Result.Fail(new ProjectTaskResponsiblePersonOnlyCanOpenTaskError());
        }

        if (task.Status == ProjectTaskStatuses.Open)
        {
            return Result.Fail(new ProjectTaskAlreadySetThisStatusError(task.Status));
        }

        task = await _projectTaskRepository.OpenProjectTask(ct, taskId, ProjectTaskStatuses.Open);
